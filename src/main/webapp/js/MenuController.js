...
/* Controllers */
var module = angular.module('adminMenu.controllers', []);
...
module.controller('MenuCtrl', ['$scope', '$filter', '$http', function ($scope, $filter, $http) {
    ...
    $scope.guardar = function () {
        ...
        if (error)
            return;
        $http.post('./webresources/Menu', JSON.stringify($scope.datosFormulario), {}
            ).success(function (data, status, headers, config) {
    ...
    $scope.eliminar = function (data){
        if (confirm('�Desea elminar este registro?')) {    
            $http.delete('./webresources/Menu/'+data.id, {})
                .success(function (data, status, headers, config) {
                    $scope.listar();
                }).error(function (data, status, headers, config) {    
                    bootbox.alert('Error al eliminar la informaci\xf3n de Menu, por favor intente m\xe1s tarde');
            });   
        }
    };
}]);
