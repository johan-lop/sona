...
/* Controllers */
var module = angular.module('adminRol.controllers', []);
...
module.controller('RolCtrl', ['$scope', '$filter', '$http', function ($scope, $filter, $http) {
    ...
    $scope.listar=function(){
        ...
        $scope.listarMenuRol=function(){
            $http.get('./webresources/MenuRol', {})
                .success(function (data, status, headers, config) {
                    $scope.listaMenuRol = data;
                }).error(function (data, status, headers, config) {
                    alert('Error al consultar la informaci\xf3n de menuRol, por favor intente m\xe1s tarde');
            });    
        };
        $scope.listarMenuRol();
